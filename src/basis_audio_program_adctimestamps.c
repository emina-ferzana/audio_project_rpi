...
static PaTime streamStartTime = 0.0;
static struct timespec utcStartStreamTime;
                          ...
                          void *userData) 
{
    ...
    PaTime inputAdcTime = timeInfo->inputBufferAdcTime - streamStartTime;
        ...
        if (data->sampleData != NULL){
            ...
            if ( inputBuffer != NULL){ 
                for (unsigned long i = 0; i < elementsToWrite; ++i) {
                    ...
                    // timestampedData[i].timestamp.tv_sec = (time_t)inputAdcTime;
                    // timestampedData[i].timestamp.tv_nsec = (long)((inputAdcTime - timestampedData[i].timestamp.tv_sec) * 1e9);
                    double precise_time = inputAdcTime + (i / (double)NUM_CHANNELS) * sample_time_interval;
...
int main(void)
{
    ...
    // Write the UTC start time to a file
    FILE *utcFile = fopen("utc_start_stream.txt", "w");
    if(utcFile != NULL) {
        fprintf(utcFile, "%ld.%09ld\n", utcStartStreamTime.tv_sec, utcStartStreamTime.tv_nsec);
        fclose(utcFile);
    } else {
        fprintf(stderr, "Error: Could not open utc_start_stream.txt for writing.\n");
    }
